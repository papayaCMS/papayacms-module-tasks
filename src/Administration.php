...
* @subpackage Tasks
* @version $Id: Administration.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class PapayaModuleTasksAdministration extends base_object {
    ...
    case 'search' :
      if ($this->requestParameters()->has('reset_search')) {
        ...
        $this->requestParameters()->remove(
          array(
            'id-starts-with', 'status', 'data-contains', 'offset',
            'time-from', 'time-to', 'task-guid'
          )
        );
      } else {
        $filters = array(
  ...
  */
  public function resetFilter() {
    ...
    foreach ($filters as $key => $val) {
      $this->papaya()->session->setValue(array($this, 'search', $key), NULL);
    }
  }
  ...
  public function setFilter($filters) {
    foreach ($filters as $key => $val) {
      if ($val === NULL || trim($val) == '' || $val === 0) {
        ...
        $this->papaya()->session->setValue(array($this, 'search', $key), $val);
      }
    }
  }
  ...
  */
  public function setOrderBy($direction = 'desc') {
    if ($direction == 'asc') {
      $this->papaya()->session->setValue(array($this, 'sort', 'created'), 'asc');
    } else {
      $this->papaya()->session->setValue(array($this, 'sort', 'created'), NULL);
    }
  }
  ...
  */
  public function setOffset($offset) {
    if ($offset >= 0) {
      $this->papaya()->session->setValue(array($this, 'offset'), (int)$offset);
    }
  }
  ...
  public function getXmlButtons() {
    if (isset($this->_selectedItem)) {
      ...
      if ($this->_selectedItem['status'] == PapayaModuleTasksItem::TASK_CREATED) {
        ...
        $button->image = $images['status-sign-ok'];
        ...
        $button->image = $images['status-sign-problem'];
        $button->reference->setParameters(
          array(
            'cmd' => 'decline',
            'task_id' => $this->_selectedItem['id']
      ...
      $button->image = $images['actions-task-delete'];
      $button->reference->setParameters(
        array(
          'cmd' => 'delete',
          'task_id' => $this->_selectedItem['id']
      ...
      $menu->elements[] = $button;
      $this->layout->addMenu($menu->getXml());
    }
  }
      ...
      if ($absoluteCount > 0) {
        ...
        foreach ($items as $item) {
          ...
          $result .= sprintf(
            ...
            PapayaUtilStringXml::escapeAttribute(
              $images[$this->_statusList[$item['status']]['icon']]
            ),
            PapayaUtilStringXml::escapeAttribute(
              $this->getLink(
                array(
          ...
          if ($item['status'] == PapayaModuleTasksItem::TASK_CREATED) {
            $result .= sprintf(
              ...
              PapayaUtilStringXml::escapeAttribute($images['status-sign-ok']),
  ...
  * @param PapayaModuleTasksItem $item
  */
  public function getXmlDetails(PapayaModuleTasksItem $item) {
    $result = '<sheet>';
    $result .= '<header><lines>';
    $result .= sprintf(
      '<line class="headertitle"><glyph src="%s" hint="%s"/>%s</line>',
      PapayaUtilStringXml::escapeAttribute(
        $this->papaya()->images[$this->_statusList[$item['status']]['icon']]
      ),
      PapayaUtilStringXml::escapeAttribute(
  ...
  */
  public function getSearchDialogXml() {
    ...
    $this->layout->addLeft($this->_dialogSearch->getDialogXml());
  }
